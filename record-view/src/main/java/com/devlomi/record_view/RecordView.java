...
public class RecordView extends RelativeLayout {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        txtSlideToCancel = view.findViewById(R.id.slide_to_cancel);
        ...
        if (attrs != null && defStyleAttr == -1 && defStyleRes == -1) {
            ...
            slideMarginRight = (int) typedArray.getDimension(R.styleable.RecordView_slide_to_cancel_margin_right, 30);
            ...
            if (slideToCancelText != null)
                txtSlideToCancel.setText(slideToCancelText);
...
//            slideToCancelLayout.startShimmerAnimation();
    ...
    protected void onActionMove(RecordButton recordBtn, MotionEvent motionEvent) {
        ...
        if (!isSwiped) {
            if (isRTLOn) {
                //Swipe To Cancel
                if (slideToCancelLayout.getX() != 0 && slideToCancelLayout.getX() >= counterTime.getLeft() - (cancelBounds * 15)) {
...
//            slideToCancelLayout.stopShimmerAnimation();
    ...
    private boolean isRTLOn = false;
    private int slideMarginRight = 0;
    ...
    private void setMarginRight(int marginRight, boolean convertToDp) {
        ...
        if (convertToDp) {
            int marginEnd = (int) DpUtil.toDp(marginRight, context);
            //layoutParams.rightMargin = marginEnd;
            Shimmer.Builder mShimmer = new Shimmer.AlphaHighlightBuilder();
            if (isRTLOn) {
                mShimmer.setDirection(Shimmer.Direction.LEFT_TO_RIGHT);
//                arrow.setRotation(180);
                layoutParams.setMargins(marginRight, 0, 0, 0);
            } else {
                mShimmer.setDirection(Shimmer.Direction.RIGHT_TO_LEFT);
//                arrow.setRotation(0);
                layoutParams.setMargins(0, 0, marginRight, 0);
            }
            slideToCancelLayout.setShimmer(mShimmer.build());
        } else
    ...
    public void setSlideToCancelText(String text) {
        txtSlideToCancel.setText(text);
    }

    public void setSlideToCancelTextColor(int color) {
        txtSlideToCancel.setTextColor(color);
    }
    ...
    public void setSlideMarginRight(int marginRight) {
        this.slideMarginRight = marginRight;
        setMarginRight(marginRight, true);
    }

    public int getSlideMargin() {
        return slideMarginRight;
    }
    ...
    public void setSlideFont(Typeface typeface){
        try {
            txtSlideToCancel.setTypeface(typeface);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public void setCounterTimerFont(Typeface typeface){
        try {
            counterTime.setTypeface(typeface);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
